var orehash = require('bindings')('orehash');

// Genesis block
var buf = Buffer.from('', 'hex');

//Should return the genesis block hash: 0000082da923a04678394f873852c7f08b777af30224b6e23296f586370e80ae
console.log("********* Lyra2REv2 Test *********");
console.log(orehash.SumLyra2REv2(buf));
console.log("^ should return \nae 80 0e 37 86 f5 96 32 e2 b6 24 02 f3 7a 77 8b f0 c7 52 38 87 4f 39 78 46 a0 23 a9 2d 08 00 00")
console.log("**********************************");
console.log("********** Lyra2RE Test **********");
console.log(orehash.SumLyra2RE(buf));
console.log("^ should return \nbf 6d 20 78 3a a0 cd 97 c9 3a 00 b6 36 65 98 b3 53 31 a6 90 1f 79 e0 ca 20 26 e5 14 a6 ed 51 ba")
console.log("**********************************");
console.log("********* Scrypt-N Test **********");
console.log(orehash.SumScryptN(buf));
console.log("^ should return \nac e1 d1 c6 13 40 29 57 4e a5 1c b9 d3 63 2f 04 a2 75 c9 82 55 af 53 24 26 3c c5 0d 75 66 8a 1f")
console.log("**********************************");
